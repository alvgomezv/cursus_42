...
/*   Updated: 2023/02/08 16:33:02 by alvgomez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	inicialize_min_and_max(t_map *map)
{
	...
	map->v->max_x = map->i->isometric_x;
	map->v->min_x = map->i->isometric_x;
	map->v->max_y = map->i->isometric_y;
	map->v->min_y = map->i->isometric_y;
}
...
void	find_min_and_max(t_map *map)
{
	...
	while (i < map->s->size_y)
	{
		while (j < map->s->size_x[i])
		{
			...
			if (map->i->isometric_x > map->v->max_x)
				map->v->max_x = map->i->isometric_x;
			if (map->i->isometric_x < map->v->min_x)
				map->v->min_x = map->i->isometric_x;
			if (map->i->isometric_y > map->v->max_y)
				map->v->max_y = map->i->isometric_y;
			if (map->i->isometric_y < map->v->min_y)
				map->v->min_y = map->i->isometric_y;
		j++;
		}
	j = 0;
